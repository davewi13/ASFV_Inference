...
// [[Rcpp::export]]
List update_exp_mean(Rcpp::DataFrame &completedata, double exp_mean, double exp_mean_sigma, double exp_shape, double exp_mean_rate, double exp_mean_shape, double Tfinal, double accept_exp_mean){
...
// It works the same way as the function for beta_ext above
  ...
  double ll_old, ll_new;
	...
	// Gamma prior
	ll_old = loglikelihood_exp_mean(completedata,exp_mean,exp_shape, Tfinal)+(exp_mean_shape-1)*log(exp_mean)-exp_mean_rate*exp_mean;
	ll_new = loglikelihood_exp_mean(completedata,prop_exp_mean,exp_shape, Tfinal)+(exp_mean_shape-1)*log(prop_exp_mean)-exp_mean_rate*prop_exp_mean;
...
// [[Rcpp::export]]
List update_exp_shape(Rcpp::DataFrame &completedata, double exp_mean, double exp_shape, double exp_shape_sigma, double exp_shape_rate, double exp_shape_shape, double Tfinal, double accept_exp_shape){
...
// It works the same way as the function for beta_ext above  
  ...
  double ll_old, ll_new;
	...
	ll_old = loglikelihood_exp_shape(completedata, exp_mean, exp_shape, Tfinal)+(exp_shape_shape-1)*log(exp_shape)-exp_shape_rate*exp_shape;
	ll_new = loglikelihood_exp_shape(completedata, exp_mean, prop_exp_shape, Tfinal)+(exp_shape_shape-1)*log(prop_exp_shape)-exp_shape_rate*prop_exp_shape;
    if(ll_new != minusinf){
		if(log(runif(1,0,1)[0]) < (ll_new-ll_old)){
...
// [[Rcpp::export]]
List update_inf_mean(Rcpp::DataFrame &completedata, double inf_mean, double inf_mean_sigma, double inf_shape, double inf_mean_rate, double inf_mean_shape, double Tfinal, double accept_inf_mean){
...
// It works the same way as the function for beta_ext above  
  ...
  double ll_old, ll_new;
	...
	// Inverse gamma prior
    ll_old = loglikelihood_inf_mean(completedata,inf_mean,inf_shape,Tfinal)+(inf_mean_shape-1)*log(inf_mean)-inf_mean_rate*inf_mean;
	ll_new = loglikelihood_inf_mean(completedata,prop_inf_mean,inf_shape,Tfinal)+(inf_mean_shape-1)*log(prop_inf_mean)-inf_mean_rate*prop_inf_mean;
	if(ll_new != minusinf){
		if(log(runif(1,0,1)[0]) < (ll_new-ll_old)){
...
// [[Rcpp::export]]
List update_inf_shape(Rcpp::DataFrame &completedata, double inf_mean, double inf_shape, double inf_shape_sigma, double inf_shape_rate, double inf_shape_shape, double Tfinal, double accept_inf_shape){
...
// It works the same way as the function for beta_ext above
  ...
  double ll_old, ll_new;
	...
	// Inverse gamma prior
	ll_old = loglikelihood_inf_shape(completedata,inf_mean,inf_shape,Tfinal)+(inf_shape_shape-1)*log(inf_shape)-inf_shape_rate*inf_shape;
	ll_new = loglikelihood_inf_shape(completedata,inf_mean,prop_inf_shape,Tfinal)+(inf_shape_shape-1)*log(prop_inf_shape)-inf_shape_rate*prop_inf_shape;
	if(ll_new != minusinf){
		if(log(runif(1,0,1)[0]) < (ll_new-ll_old)){
